...
public class AccountingServiceImpl implements AccountingService {
	...
	void detailsManagerPopulation() {
		LOG.debug("detailsManagerPopulation launched");
		List<AccountEntity> restoredDocs = accounts.findByExpirationGreaterThanAndRevokedIsFalse(LocalDateTime.now());
		LOG.debug("accounts {} has been took from repo", accounts.count());
		for (AccountEntity acc : restoredDocs) {
			if (!acc.isRevoked()) {
				userDetailsManager.createUser(
